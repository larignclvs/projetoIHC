...
> **Atenção:** Concorrente não é necessariamente idêntico, e sim alguém atuando na mesma área.
...
[Posture Corrector](https://posturecorrector.vercel.app/)  
[Facial Expression Analysis (iMotions)](https://imotions.com/products/imotions-lab/modules/fea-facial-expression-analysis/?srsltid=AfmBOoqE8ZPi2L1aJV5Nyal3lGM3_nMomMQEA23iQndPiFJB9cYUcCK9)  
[MorphCast for Zoom](https://www.morphcast.com/morphcast-for-zoom/)  
...
![WhatsApp Image 2025-09-07 at 22 10 58](https://github.com/user-attachments/assets/d806329a-c97b-4842-8a7c-12e7da2643f5)
...
| 2. Exibir pop-up de alerta                 |        |                                          | Exibir pop-up para status "Inquieto" (2.1) ou exibir pop-up para status "Cansado" (2.2).                 | Incluir um botão de "pausar monitoramento" no pop-up para facilitar a interação. |
| 2.1 Exibir pop-up para status "Inquieto" |                                 |                                         |                                                                                                              |                                                                                    |
| 2.2 Exibir pop-up para status "Cansado"  |                                      | |                                                                                                              |                                                                                    |
| 3. Fechar o pop-up                         |                          |                                                                                |                                                                                                              |                                                                                    |
...
| Objetivos/Operações                             | Input                                                                                   | Feedback                                                                                                                   | Plano                                                                                                     | Recomendação                                                                                  |
|-------------------------------------------------|-----------------------------------------------------------------------------------------|----------------------------------------------------------------------------------------------------------------------------|-----------------------------------------------------------------------------------------------------------|-----------------------------------------------------------------------------------------------|
| 0. Visualizar relatório final do monitoramento  | Usuário encerra a sessão de monitoramento e deseja ver um resumo completo da sessão.   | Tela final é exibida com estatísticas, gráficos, alertas e opções de exportação/fechamento.                              | Encerrar sessão (1) → Gerar relatório (2) → Exibir tela com relatório (3) → Exportar (4) → Fechar (5).   | Destacar visualmente que a sessão foi concluída e que os dados exibidos são daquele período.  |
| 1. Encerrar sessão de monitoramento             | Usuário clica em “Parar monitoramento”    | Sistema confirma o fim da sessão (mensagem breve) e interrompe a captura/análise em tempo real.                          | Ao receber comando de parada, finalizar captura de dados e consolidar registros da sessão.               |                          |
| 2. Gerar relatório                              | Sistema recebe os dados consolidados da sessão (expressões, postura, alertas, tempo).  |                                         | Gerar estatísticas (2.1), montar gráficos (2.2) e compilar alertas (2.3) antes de exibir a tela final.   | Manter o processamento rápido e indicar claramente que o relatório está sendo preparado.     |
| 2.1 Gerar estatísticas                          | Dados agregados da sessão (tempo total, níveis médios, picos, frequências de eventos). | Tabelas/resumos numéricos com métricas de estresse, cansaço, inquietação e duração da sessão.                            | Calcular estatísticas globais e por período (ex.: início, meio e fim da sessão).                         |   |
| 2.2 Montar gráficos                             | Séries temporais, eventos detectados e níveis de classificação ao longo do tempo.      | Gráficos de linha, barras ou áreas destacando variações de estresse, cansaço e inquietação durante a sessão.             | Transformar estatísticas em visualizações legíveis e coerentes com o restante da interface.             | Usar cores consistentes com o sistema e legendas claras para facilitar a interpretação.      |
| 2.3 Compilar alertas feitos durante a sessão    | Lista de alertas disparados (tipo, horário, mensagem, gravidade).                      | Seção no relatório listando os alertas em ordem cronológica, com ícones ou cores que indiquem criticidade.               | Agrupar alertas por tipo e horário            | Destacar apenas os alertas mais importantes e permitir rolagem para listas muito longas.     |
| 3. Exibir tela final com relatório              | Relatório gerado (estatísticas, gráficos e alertas) pronto para visualização.          | Tela final de relatório com layout organizado em seções (resumo, gráficos, alertas, opções de exportar/fechar).          | Carregar o relatório em uma tela dedicada, em modo somente leitura, com rolagem e navegação clara.       |          |
| 4. Exportar relatório completo                  | Usuário clica em “Exportar relatório”                           | Confirmação de sucesso (“Relatório exportado para…”) ou mensagem de erro com orientação em caso de falha.                | Converter relatório em arquivo (ex.: PDF), salvar em local escolhido e registrar sucesso/falha.          |               |
| 5. Fechar tela de relatório                     | Usuário clica em “Fechar”.                      | Tela de relatório é fechada e o usuário retorna à tela inicial ou dashboard principal do sistema.                        | Encerrar apenas a visualização do relatório, mantendo a aplicação ativa e pronta para nova sessão.       | Evitar perda de dados: garantir que o relatório já foi gerado/exportado antes de fechar.     |
...
<img width="660" height="554" alt="image" src="https://github.com/user-attachments/assets/d8c75bc9-19b3-43d1-aebf-12f135f1f884" />
| Objetivos/Operações                                  | Input                                                                                     | Feedback                                                                                          | Plano                                                                                                 | Recomendação                                                                                  |
|------------------------------------------------------|-------------------------------------------------------------------------------------------|---------------------------------------------------------------------------------------------------|-------------------------------------------------------------------------------------------------------|--------------------------------------------------------------------------------------------------|
| 0. Consultar status na janela flutuante              | O sistema atualiza o status do usuário para "Estressado" ou "Neutro".                    | A janela flutuante exibe o status atual com texto e elementos visuais (cor/ícone).                | Atualizar status do usuário (1), exibir janela flutuante (2) e interpretar status exibido (3).        | A janela deve ser fixa, visível, mas não intrusiva.                                             |
| 1. Atualizar status do usuário                       | Dados capturados pela câmera são analisados pelo modelo em tempo real.                   |                                                                                                   |                                                                                                       | Importante garantir precisão na detecção para evitar alertas incorretos.                      |
| 2. Exibir janela flutuante de status                 | Sistema recebe o status classificado.                                                     | A janela aparece ou é atualizada com o status.                                                    | Exibir status estressado (2.1) ou exibir status neutro (2.2).                                         | Utilizar cores ou ícones para facilitar a compreensão visual rápida.                          |
...
### Nome do Cenário
Victor monitora o próprio nível de estresse enquanto programa
...
### **1) Avaliação de IHC através de inspeção HEURÍSTICA (Gabi)**  
...
**Local:** Estados de fadiga (cansado, inquieto, focado)  
...
### **Estudantes**  
Apesar de apresentarem leve dificuldade para identificar a função de alguns botões e compreender certos termos técnicos, os estudantes conseguiram executar a maior parte das tarefas de forma satisfatória. Observou-se, porém, que esse público tende a depender mais de orientações visuais claras e feedback imediato, especialmente em ações como iniciar ou pausar o monitoramento. A falta de um guia inicial também prolongou o tempo necessário para completar tarefas simples, reforçando a importância de um tutorial rápido e objetivo para facilitar o primeiro uso e reduzir erros por interpretação incorreta.  
...
## 7) **Conclusão Final**  
A observação dos usuários revelou que a usabilidade do sistema depende principalmente da clareza visual, da consistência dos termos usados e da visibilidade do estado de monitoramento. Usuários iniciantes e experientes apresentaram dificuldades semelhantes para identificar ações essenciais, principalmente por falta de feedback imediato e rótulos intuitivos. Usuários iniciantes (como Nicole e estudantes) demonstram maior dependência de orientações claras, ícones explicativos e tutorial inicial, já usuários avançados (como Victor e profissionais em home office) valorizam precisão, rapidez e consistência semântica, e podem ter sua produtividade prejudicada por ambiguidades simples.
